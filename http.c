...
struct Response http_get(const char *url) {
    ...
    if (CURLE_OK !=
        (res = curl_easy_setopt(resp._curl, CURLOPT_FOLLOWLOCATION, true))) {
    ...
    if (CURLE_OK != (res = curl_easy_setopt(resp._curl,
    ...
    if (CURLE_OK !=
        (res = curl_easy_setopt(resp._curl, CURLOPT_WRITEDATA, &resp))) {
